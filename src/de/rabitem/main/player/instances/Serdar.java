...
public class Serdar extends Player {
    ...
    private int bestMove(Game game, int pointCardValue) {
        ...
        for (int i = 0; i < game.pointCards.size(); i++) {
            ...
            int score = minimax(copyGame, 0, false);
            if (score > bestScore) {
                bestScore = score;
                move = i;
            }
    ...
    private int minimax(Game game, int depth, boolean isMaximizing) {
        ...
        if (isMaximizing) {
            ...
            for (int i = 0; i < copyGame.cardsP1.size(); i++) {
                for (int j = 0; j < copyGame.pointCards.size(); j++) {
                    copyGame = new Game(game);
                    copyGame.play(copyGame.cardsP1.get(i).getValue(), copyGame.pointCards.get(j).getValue(), true);
                    int score = minimax(copyGame, depth+1, false);
                    /*
                    if (score < 0)
                        scoreAddition = scoreAddition - 1;
                    else if (score > 0)
                        scoreAddition = scoreAddition + 1;
            ...
            for (int i = 0; i < copyGame.cardsP2.size(); i++) {
                for (int j = 0; j < copyGame.pointCards.size(); j++) {
                    copyGame = new Game(game);
                    copyGame.play(copyGame.cardsP2.get(i).getValue(), copyGame.pointCards.get(j).getValue(), false);
                    int score = minimax(copyGame, depth+1, true);
                    /*
                    if (score < 0)
                        scoreAddition = scoreAddition - 1;
                    else if (score > 0)
